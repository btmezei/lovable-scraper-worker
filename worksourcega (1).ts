...
//
// v5.2 — fast statewide page-cap build. We still pull statewide and filter
...
export const WORKSOURCE_GA_WORKER_VERSION = "v5.2-fast-page-cap-2026-07-29";
...
export async function runWorksourceGa(
  ...
  const diag: string[] = [];
  log(diag, `${WORKSOURCE_GA_WORKER_VERSION} — fast statewide page-cap mode`);
  ...
  try {
    ...
    const MAX_PAGES = FAST_MAX_PAGES;
    let consecutiveNoGain = 0;
    let currentPage = 1;
    for (let p = 2; p <= MAX_PAGES; p++) {
      if (!hasBudget()) { log(diag, `runtime budget reached before page ${p}; returning partial batch`); break; }
    ...
    if (currentPage >= MAX_PAGES) log(diag, `page cap ${MAX_PAGES} reached intentionally; returning fast batch`);
