...
export const VOS_WORKER_VERSION = "vos-worker-v1.7-pagelen-diag-2026-08-10";
...
export async function runVosWorker(input: VosWorkerInput): Promise<VosWorkerResult> {
  ...
  try {
    ...
    for (const size of ["100", "50", "25"]) {
      ...
      await safeEvaluate(browser!, activeRef, () => {
        const toggle = document.querySelector<HTMLElement>(
          '[id*="PageLen" i][data-toggle="dropdown"], [id*="PageLen" i] .dropdown-toggle, a.dropdown-toggle[id*="PageLen" i], [id*="PageLenMenu" i]',
        );
        toggle?.click();
      }).catch(() => null);
      await sleep(600);

      const clicked = await clickAndSettle(
        browser!,
        activeRef,
        (sz: string) => {
          const items = Array.from(
            document.querySelectorAll<HTMLElement>(
              '[id*="PageLen" i] a, div.CardListPagerDD a.dropdown-item, a.dropdown-item',
            ),
          );
    ...
    // In diagnostic mode we only need page 1 markup — skip the expensive walk.
    const walkCap = input.pagerDiag ? 1 : maxPages;
