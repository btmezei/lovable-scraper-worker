...
import { runVosWorker, VOS_WORKER_VERSION, proxyConfigured } from "./vos.js";
...
app.get("/healthz", (_req, res) => {
  res.json({
    ...
    // Readiness flag for the .gov lane (CalJOBS et al.): true once
    // VOS_PROXY_URL is set on this host. No credential is ever echoed.
...
const MAX_DISK_MIRROR_BYTES = 8 * 1024 * 1024;

function persist(job: JobState) {
  try {
    // Deep slices carry many MB of page payload. Serializing the full result to
    // disk on a 512MB Render instance is exactly what OOM-killed the process at
    // the end of a 20-page walk, so mirror metadata only when the body is big —
    // the in-memory copy still answers the poll.
    let body = JSON.stringify(job);
    if (body.length > MAX_DISK_MIRROR_BYTES) {
      body = JSON.stringify({ ...job, result: undefined, resultTooLargeForDisk: true });
    }
    fs.writeFileSync(jobPath(job.id), body);
  } catch {
    /* best-effort */
  }
}
...
app.post("/jobs/hydrate-vos", (req, res) => {
  ...
  const job = startJob("hydrate-vos", (onProgress) => runVosHydrate(parsed.data));
