...
export const VOS_HYDRATE_WORKER_VERSION = "vos-hydrate-v1.5-apply-restable-2026-08-10";
 ...
 */
/**
 * v1.5: the DOM is re-rendered by ASP.NET UpdatePanels shortly after load, so a
 * handle taken during the first tagging pass can be detached by the time we
 * click it ("detached frame" / "stale node"). We now let the page settle, then
 * re-tag and re-query the control immediately before each click, retrying once
 * when the node goes stale mid-click.
 */
...
/** Tag the Apply control in whichever frame holds it; returns what was seen. */
async function tagApplyControl(
  page: Page,
): Promise<{ seen: string[]; targetFrame: import("puppeteer-core").Frame | null }> {
  const seen: string[] = [];
  ...
  for (const frame of page.frames()) {
    ...
    const res = await frame
      .evaluate(() => {
        ...
        document
          .querySelectorAll("[data-vos-apply]")
          .forEach((el) => el.removeAttribute("data-vos-apply"));
  ...
  return { seen, targetFrame };
}

const isStaleError = (msg: string) =>
  /detached|not attached|stale|Node is either not clickable|Execution context was destroyed|Target closed/i.test(
    msg,
  );
...
async function resolveApplyDestination(
  ...
  try {
    // 1) Let async UpdatePanel re-renders finish, then find the Apply control.
    await settle(page);
    let { seen, targetFrame } = await tagApplyControl(page);
    if (!targetFrame) {
      // One more chance after a longer settle — late-rendering toolbars.
      await settle(page, 1_200);
      ({ seen, targetFrame } = await tagApplyControl(page));
    }
    ...
    // Re-query the handle right before clicking; retry once if it goes stale.
    let clicked = false;
    let lastClickError = "";
    for (let attempt = 0; attempt < 2 && !clicked; attempt++) {
      if (attempt > 0) {
        await settle(page, 500);
        const retag = await tagApplyControl(page);
        if (!retag.targetFrame) break;
        targetFrame = retag.targetFrame;
      }
      if (targetFrame.isDetached?.()) {
        lastClickError = "detached frame";
        continue;
      }
      const handle = await targetFrame.$("[data-vos-apply='1']").catch(() => null);
      if (!handle) {
        lastClickError = "apply-control-vanished";
        continue;
      }
      ...
      lastClickError = err;
      // Covered by a sticky toolbar? Fall back to an in-page DOM click.
      const domClicked = await targetFrame
        .evaluate(() => {
          const el = document.querySelector<HTMLElement>("[data-vos-apply='1']");
          if (!el) return false;
          el.click();
          return true;
        })
      ...
      if (!isStaleError(err)) break;
    }

    if (!clicked) {
      return {
        applyResolution: lastClickError.includes("vanished")
          ? "apply-control-vanished"
          : `apply-click-error: ${lastClickError.slice(0, 80)}`,
        applyDebug,
      };
    }
