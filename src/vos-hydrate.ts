...
export const VOS_HYDRATE_WORKER_VERSION = "vos-hydrate-v1.1-parallel-tabs-2026-08-09";

const NAV_TIMEOUT_MS = 45_000;
/**
 * v1.0 walked detail pages one at a time in a single tab: ~30s per page, so a
 * 60-row batch could never return inside the app's request ceiling and the
 * whole batch was discarded without committing a single row. Bright Data
 * sessions support many concurrent tabs, so hydrate a pool of pages at once.
 */
...
export type VosHydrateInput = {
  ...
  urls: string[];
  budgetMs?: number;
  /** Parallel tabs per batch. Defaults to 8, hard-capped at 16. */
  concurrency?: number;
};
...
async function readDetailPage(page: Page, url: string, host: string): Promise<VosHydratePage> {
  try {
    ...
    await page.waitForNetworkIdle({ timeout: 3_000 }).catch(() => null);
...
export async function runVosHydrate(input: VosHydrateInput): Promise<VosHydrateResult> {
  ...
  try {
    ...
    // Fan the batch out over a pool of tabs that share the seeded session.
    const concurrency = Math.max(
      1,
      Math.min(MAX_CONCURRENCY, Math.floor(Number(input.concurrency) > 0 ? Number(input.concurrency) : DEFAULT_CONCURRENCY)),
    );
    log(diag, `hydrating with ${concurrency} parallel tabs`);
    ...
    const worker = async (slot: number) => {
      // Slot 0 reuses the already-open, session-seeded tab.
      const tab = slot === 0 ? page : await browser!.newPage();
      if (slot !== 0) {
        tab.setDefaultNavigationTimeout(NAV_TIMEOUT_MS);
        tab.setDefaultTimeout(NAV_TIMEOUT_MS);
      }
      try {
        while (true) {
          const index = cursor++;
          ...
          results[index] = await readDetailPage(tab, urls[index]!, input.host);
        }
      } finally {
        if (slot !== 0) await tab.close().catch(() => null);
      }
    };

    await Promise.all(
      Array.from({ length: Math.min(concurrency, urls.length) }, (_unused, slot) => worker(slot)),
    );
