...
export const VOS_WORKER_VERSION = "vos-worker-v1.5-async-jobs-2026-08-10";
...
export async function runVosWorker(input: VosWorkerInput): Promise<VosWorkerResult> {
  ...
  try {
        ...
        (sz: string) => {
          const items = Array.from(
            document.querySelectorAll<HTMLElement>(
              "div.CardListPagerDD a.dropdown-item, a.dropdown-item, a, span[role='button']",
            ),
          );
    ...
    for (let p = 2; p <= maxPages; p++) {
      ...
      const fp = await fingerprint();
      const advanced = await safeEvaluate(
        browser!,
        activeRef,
        (want: number) => {
      ...
      // Re-attach after the pager click in case the portal navigated.
      try {
