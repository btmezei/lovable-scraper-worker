...
export const VOS_WORKER_VERSION = "vos-worker-v1.8-pagelen-exec-2026-08-10";
...
/** Evaluate on the active page, re-acquiring the handle once if the target closed. */
...
async function clickAndSettle(
  ...
  try {
    ...
    log(diag, `${label} click failed: ${e instanceof Error ? e.message : String(e)}`);
    return false;
  }
  ...
  try {
    ...
    log(diag, `${label} could not re-acquire page: ${e instanceof Error ? e.message : String(e)}`);
    return false;
  }
...
export async function runVosWorker(input: VosWorkerInput): Promise<VosWorkerResult> {
  ...
  try {
    ...
    if (input.pagerDiag) {
      ...
      }).catch((e) => `pagelen capture failed: ${e instanceof Error ? e.message : String(e)}`);
    ...
    for (const size of ["100", "50", "25", "20"]) {
      ...
      // The size options are Bootstrap dropdown items whose real behaviour lives
      // in an inline onclick like `Card_lst_..._CardList.pageLen(100)`. Clicking a
      // hidden item is unreliable, so execute that handler directly in page
      // context (falling back to a click if no onclick attribute exists).
      const fired = await clickAndSettle(
        browser!,
        activeRef,
        (sz: string) => {
          const items = Array.from(
            document.querySelectorAll<HTMLElement>(
              '[id*="PageLen" i] a, div.CardListPagerDD a, a.dropdown-item, [class*="CardListPagerDD"] a',
            ),
          );
      ...
      if (fired && (await waitForGridChange(fp))) {
