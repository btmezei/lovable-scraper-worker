...
export const VOS_WORKER_VERSION = "vos-worker-v2.5-gov-proxy-lane-2026-09-06";
...
async function openBrowser(
  ...
  const mustUseProxy = isGovHost(host);

  if (proxy && (mustUseProxy || !wss)) {
    log(diag, `launching local Chromium via proxy ${proxy.server}${mustUseProxy ? " (.gov host)" : ""}`);
  ...
  if (!wss) throw new Error("BRIGHTDATA_SCRAPING_BROWSER_WSS not configured");
  if (mustUseProxy) {
    throw new Error(
      `${host} is a .gov host: Bright Data refuses it by policy. Set VOS_PROXY_URL to a residential proxy.`,
    );
  }
...
export async function runVosWorker(
  ...
  let launched = false;
  try {
    // `.gov` portals (CalJOBS, VA/MD/MO/TN) are refused by Bright Data policy,
    // so those hosts must go through a local Chromium pointed at a plain
    // residential proxy. Everything else keeps the Bright Data session.
    ...
    pagesGz.push(pack(await grabHtml()));
    currentPage = 1;
    emit("page 1 captured");
    ...
    for (let p = 2; p <= walkCap; p++) {
      ...
      try {
        ...
        currentPage = p;
        pagesGz.push(pack(await grabHtml()));
        emit(`page ${p} captured`);
      } catch (e) {
        // The session died mid-walk (detached frame / target closed). Keep every
        // page already captured and stop cleanly instead of losing the slice.
        log(diag, `walk aborted at page ${p}: ${describeErr(e)}`);
        emit(`walk aborted at page ${p}`);
        break;
      }
    }
    ...
    log(diag, `walked ${currentPage} pages, ${pagesGz.length} html blobs, total="${totalText ?? "?"}"`);
    ...
    await shutdown(browser, launched);
    // PARTIAL SUCCESS: if we captured pages before the session died, ship them.
    // A detached frame on page 12 must not erase pages 1-11.
    if (pagesGz.length > 0) {
      log(diag, `session died after ${currentPage} pages — returning partial: ${describeErr(e)}`);
      return {
        workerVersion: VOS_WORKER_VERSION,
        status: "ok",
        pagesHtml: [],
