...
//
// v3 — Statewide pull, filter locally.
//
...
export const WORKSOURCE_GA_WORKER_VERSION = "v4.2-statewide-direct-2026-07-27";
...
export async function runWorksourceGa(
  ...
  const diag: string[] = [];
  log(diag, `${WORKSOURCE_GA_WORKER_VERSION} — statewide direct joblist mode; no Change Area / Set Area flow exists in this build`);
  ...
  try {
    ...
    if (!usePasted) {
      // Seed a fresh guest session — the portal sets ASP.NET_SessionId +
      // GSILB cookies here that the joblist page needs.
    ...
    // Statewide can be many pages (5k+ jobs = ~200 pages of 25). Cap high
    // but bail early when we stop gaining new cards.
    const MAX_PAGES = 250;
    let consecutiveNoGain = 0;
    for (let p = 2; p <= MAX_PAGES; p++) {
      const before = allCards.length;
      const advanced = await page.evaluate(() => {
        // 1. ASP.NET pager: <a href="javascript:__doPostBack('...','Page$N')">
        ...
        // 2. Numeric pager: find current page and click page N+1
