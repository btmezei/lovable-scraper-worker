...
export async function runWorksourceGa(
  ...
  try {
    ...
    const setAreaInfo = await page.evaluate(() => {
      ...
      return {
        id: btn.id, tag: btn.tagName, onclick: onclick.slice(0, 200),
        target: m ? m[1] : null, arg: m ? m[2] : null,
      };
    });
    log(diag, `setArea info: ${JSON.stringify(setAreaInfo)}`);

    if (setAreaInfo?.target) {
      // Fire the real postback and wait for the UpdatePanel to swap in radius.
      ...
      log(diag, `radius appeared after postback: ${appeared}`);
    ...
    if (radiusInfoFinal) {
      ...
      const panelHtml = await page.evaluate(() => {
        ...
        return panel ? (panel as HTMLElement).outerHTML.slice(0, 4000) : null;
      });
